                    ...
                    .SelectMany(x =>
                    {
                        ...
                        return Regex.Matches(query, x.OpenMarker + QueryParams.RegexPattern + x.CloseMarker)
                            ...
                            .Select(varName => new
                            {
                                VarName = varName,
                                DbParamName =
                                // if the parameter name is not in the dictionary, add it with a count of 1 and return the 1
                                // otherwise, return the current count of the parameter name and incrument the count by 1
                                // ^ the above functionality does not translate well to the suggested warning CA1854 usage of .TryGetValue()
                                // when the logic is meant to fit within one line trinary operator
